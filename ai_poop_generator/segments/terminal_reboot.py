...
def gen_terminal_reboot_segment(
    ...
    for _ in range(15):
        img = rgb_static_noise(constants.WIDTH, constants.HEIGHT)
    ...
    for _ in range(15):
        img = crt_warmup_noise(constants.WIDTH, constants.HEIGHT)
    ...
    for f in range(30):
        ...
        draw.text((80, constants.HEIGHT // 2 - 50), "READY.", font=ready_font, fill=(0, green, 0))
        # Blinking cursor
        if f % 10 < 5:
            cursor_x = 80 + int(draw.textlength("READY. ", font=ready_font))
            draw.rectangle((cursor_x, constants.HEIGHT // 2 - 50, cursor_x + 20, constants.HEIGHT // 2), fill=(0, green, 0))
    ...
    for f in range(21):
        ...
        draw = ImageDraw.Draw(img)
        # Keep READY visible
        draw.text((80, constants.HEIGHT // 2 - 50), "READY.", font=ready_font, fill=(0, 255, 0))
        # Fade in the ended message
        alpha = min(1.0, f / 10)
        red = int(200 * alpha)
        draw.text((80, constants.HEIGHT // 2 + 30), "[conversation ended]", font=end_font, fill=(red, 0, 0))
        img = scanlines(img, gap=3, alpha=20)
        img.save(os.path.join(frame_dir, f"frame_{frame_idx:05d}.png"))
        frame_idx += 1
